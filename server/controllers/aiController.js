...
export const generateThumbnail = async (req, res) => {
    try {
        ...
        if (!title || !title.trim()) {
            return res.json({ success: false, message: "Please provide a video title." })
        }

        const count = clampNumber(variations, 1, 4, 1);
        const prompt = [
            `Create a YouTube thumbnail at 1280x720 for the video title: "${title}".`,
            visual_prompt ? `Visual direction: ${visual_prompt}.` : '',
            `Template: ${template}. Emotion: ${emotion}.`,
            text_overlay ? `Use bold readable text overlay: "${text_overlay}".` : 'Leave clean space for bold readable title text.',
            face_emphasis ? 'Emphasize face/eyes and expression if a person is present.' : '',
            high_contrast ? 'Use high-contrast colors and strong visual hierarchy.' : '',
            background_effects ? 'Use background isolation, glow, arrows, or depth effects where helpful.' : '',
            ctr_optimized ? 'Optimize for high click-through rate, mobile readability, and clear focal point.' : ''
        ].filter(Boolean).join(' ');
        ...
        for (let index = 0; index < count; index += 1) {
            const formData = new FormData()
            formData.append('prompt', `${prompt} A/B variation ${index + 1}.`)
            const { data } = await axios.post("https://clipdrop-api.co/text-to-image/v1", formData, {
                headers: { 'x-api-key': process.env.SECRET_API_KEY },
                responseType: "arraybuffer",
            })
            ...
            await sql` INSERT INTO creations (user_id, prompt, content, type) VALUES (${userId}, ${prompt}, ${secure_url}, 'youtube-thumbnail') `;
        }

        res.json({
            success: true,
            content: count === 1 ? thumbnails[0] : thumbnails,
            message: count === 1 ? "Thumbnail generated successfully" : `${count} thumbnail variations generated successfully`
        });
    } catch (error) {
...
const getStyleTransformation = (style, intensity) => {
    const strength = clampNumber(intensity, 1, 100, 65);

    switch (style) {
        case 'Anime':
            return [{ effect: `cartoonify:${strength}` }, { effect: "saturation:20" }];
        case 'Oil Painting':
            return [{ effect: `oil_paint:${strength}` }];
        case 'Watercolor':
            return [{ effect: "art:hokusai" }, { effect: "saturation:15" }];
        case 'Sketch':
            return [{ effect: "sketch" }, { effect: "auto_contrast" }];
        case 'Cartoon':
            return [{ effect: `cartoonify:${strength}` }, { effect: "saturation:20" }];
        case 'Cyberpunk':
            return [{ effect: "colorize:35:8A2BE2" }, { effect: "saturation:40" }, { effect: "contrast:20" }];
        default:
            return [{ effect: "improve" }];
    }
}
...
export const styleTransfer = async (req, res) => {
    try {
        ...
        const referenceImage = req.files?.reference_image?.[0];
        const {
            primary_style = 'Anime',
            secondary_style = '',
            tertiary_style = '',
        ...
        const count = clampNumber(batch_count, 1, 4, 1);
        const styles = [primary_style, secondary_style, tertiary_style].filter(Boolean);
        const outputs = [];
        let referenceUpload = null;

        if (referenceImage) {
            referenceUpload = await cloudinary.uploader.upload(referenceImage.path);
        }

        for (let index = 0; index < count; index += 1) {
            const style = styles[index % styles.length] || primary_style;
            let transformation = getStyleTransformation(style, intensity);

            if (scope === 'Subject Only') {
                transformation.unshift({ effect: 'background_removal', background_removal: 'remove_the_background' });
            } else if (scope === 'Background Only') {
                transformation.unshift({ effect: `gen_background_replace:prompt_${style} styled background` });
            }

            if (face_aware === 'true' || face_aware === true) {
                transformation.push({ effect: "gen_face_enhance" });
            }

            if (preserve_details === 'true' || preserve_details === true) {
                transformation.push({ effect: "sharpen:30" });
            }

            if (high_resolution === 'true' || high_resolution === true) {
                transformation.push({ width: "iw_mul_2.0", crop: "scale" });
            }

            if (nft_export === 'true' || nft_export === true) {
                transformation.push({ width: 1600, height: 1600, crop: "fill", gravity: "auto" });
            }

            transformation.push({ quality: "auto:best" }, { fetch_format: "png" });

            const { secure_url } = await cloudinary.uploader.upload(image.path, { transformation });
            outputs.push(secure_url);

            const prompt = `Style transfer - Styles: ${styles.join(', ') || primary_style}, Applied: ${style}, Intensity: ${intensity}, Scope: ${scope}${referenceUpload ? ', Custom reference used' : ''}`;
            await sql` INSERT INTO creations (user_id, prompt, content, type) VALUES (${userId}, ${prompt}, ${secure_url}, 'style-transfer') `;
        }

        res.json({
            success: true,
            content: count === 1 ? outputs[0] : outputs,
            message: count === 1 ? "Style transfer completed successfully" : `${count} stylized exports completed successfully`
        });
    } catch (error) {
        console.log('Style Transfer Error:', error.message)
        res.json({ success: false, message: error.message })
    }
}
